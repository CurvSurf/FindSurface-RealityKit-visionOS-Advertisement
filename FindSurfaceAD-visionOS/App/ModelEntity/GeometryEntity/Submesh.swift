...
extension Submesh {
    ...
    static func generateSphericalPatch(
        radius: Float,
        ...
        let horizontalAngle = min(max(horizontalAngle, 0), 2.0 * Float.pi)
        let residualHorizontalAngle = horizontalAngle.truncatingRemainder(dividingBy: unitAngle) * 0.5
        let horizontalAngleHasResidualAngle = residualHorizontalAngle >= .pi / 180
        let horizontalAngleCount = Int((horizontalAngle / unitAngle).rounded(.towardZero))
        let horizontalHalfAngle = horizontalAngle * 0.5 - residualHorizontalAngle
        var horizontalAngles = (0...horizontalAngleCount).map { unitAngle * Float($0) - horizontalHalfAngle }
        if horizontalAngleHasResidualAngle,
           let firstAngle = horizontalAngles.first,
           let lastAngle = horizontalAngles.last {
            horizontalAngles = [firstAngle - residualHorizontalAngle] +
            horizontalAngles + [lastAngle + residualHorizontalAngle]
        }
        
        let verticalAngle = min(max(verticalAngle, 0), Float.pi)
        let residualVerticalAngle = verticalAngle.truncatingRemainder(dividingBy: unitAngle) * 0.5
        let verticalAngleHasResidualAngle = residualVerticalAngle >= .pi / 180
        let verticalAngleCount = Int((verticalAngle / unitAngle).rounded(.towardZero))
        let verticalHalfAngle = verticalAngle * 0.5 - residualVerticalAngle
        var verticalAngles = (0...verticalAngleCount).map { unitAngle * Float($0) - verticalHalfAngle + .pi * 0.5 }
        if verticalAngleHasResidualAngle,
           let firstAngle = verticalAngles.first,
           let lastAngle = verticalAngles.last {
            verticalAngles = [firstAngle - residualVerticalAngle] +
            verticalAngles + [lastAngle + residualVerticalAngle]
        }
        ...
        let firstVerticalAngle = verticalAngles.first!
        let lastVerticalAngle = verticalAngles.last!
        let diffVerticalAngle = lastVerticalAngle - firstVerticalAngle
        
        let firstHorizontalAngle = horizontalAngles.first!
        let lastHorizontalAngle = horizontalAngles.last!
        let diffHorizontalAngle = lastHorizontalAngle - firstHorizontalAngle
        
        for verticalAngle in verticalAngles {
            let sinVerticalAngle = sin(verticalAngle)
            let cosVerticalAngle = cos(verticalAngle)
            
            let v = 1.0 - (verticalAngle - firstVerticalAngle) / diffVerticalAngle
            
            for horizontalAngle in horizontalAngles {
                let sinHorizontalAngle = sin(horizontalAngle)
                let cosHorizontalAngle = cos(horizontalAngle)
                
                let x = sinVerticalAngle * cosHorizontalAngle
                let y = cosVerticalAngle
                let z = sinVerticalAngle * -sinHorizontalAngle
                
                let normal = simd_float3(x, y, z)
                
                positions.append(normal * radius)
                normals.append(normal)
                
                let u = (horizontalAngle - firstHorizontalAngle) / diffHorizontalAngle
        ...
        let vertexCountPerRow = horizontalAngles.count

        for v in verticalAngles.indices.dropLast() {
            for h in horizontalAngles.indices.dropLast() {
                let upperLeft = UInt32(v * vertexCountPerRow + h)
                let lowerLeft = UInt32((v + 1) * vertexCountPerRow + h)
                let lowerRight = UInt32((v + 1) * vertexCountPerRow + (h + 1))
                let upperRight = UInt32(v * vertexCountPerRow + (h + 1))

                indices.append(contentsOf: [upperLeft, lowerLeft, lowerRight])
                indices.append(contentsOf: [upperLeft, lowerRight, upperRight])
            }
        }

        return .init(positions: positions, normals: normals, texcoords: texcoords, triangleIndices: indices)
    }
    ...
    static func generateSphericalPatch(
        ...
        return generateSphericalPatch(radius: radius,
                                      horizontalAngle: horizontalAngle,
                                      verticalAngle: horizontalAngle / aspectRatio,
                                      unitAngle: unitAngle)
    }
    
    static func generateVolumetricSphericalPatch(
        radius: Float,
        horizontalAngle: Float,
        verticalAngle: Float,
        unitAngle: Float,
        ...
        let outlineMarginAngle = outlineMargin / radius
        ...
        if let firstAngle = horizontalAngles.first,
           let lastAngle = horizontalAngles.last {
            horizontalAngles = [firstAngle - outlineMarginAngle] +
            horizontalAngles + [lastAngle + outlineMarginAngle]
        }
        ...
        if let firstAngle = verticalAngles.first,
           let lastAngle = verticalAngles.last {
            verticalAngles = [firstAngle - outlineMarginAngle] +
            verticalAngles + [lastAngle + outlineMarginAngle]
        }
        
        let horizontalAnglesTrig = horizontalAngles.map { (sin($0), cos($0)) }
        let verticalAnglesTrig = verticalAngles.map { (sin($0), cos($0)) }
        
        let horizontalAngleTrigFirst = horizontalAnglesTrig.first!
        let horizontalAngleTrigLast = horizontalAnglesTrig.last!
        let verticalAngleTrigFirst = verticalAnglesTrig.first!
        let verticalAngleTrigLast = verticalAnglesTrig.last!
        
        let innerRadius = radius - outlineMargin
        let outerRadius = radius + outlineMargin
        
        let topSurface: Submesh = {
            var positions = [simd_float3]()
            var normals = [simd_float3]()
            var indices = [UInt32]()
            
            let (sinVerticalAngle, cosVerticalAngle) = verticalAngleTrigFirst
            
            for (sinHorizontalAngle, cosHorizontalAngle) in horizontalAnglesTrig {
                
                let x = sinVerticalAngle * cosHorizontalAngle
                let y = cosVerticalAngle
                let z = sinVerticalAngle * -sinHorizontalAngle
                
                let basePosition = simd_float3(x, y, z)
                let innerPosition = basePosition * innerRadius
                let outerPosition = basePosition * outerRadius
                
                let dx = -cosVerticalAngle * cosHorizontalAngle
                let dy = sinVerticalAngle
                let dz = -cosVerticalAngle * -sinHorizontalAngle
                
                let normal = simd_float3(dx, dy, dz)
            ...
            for i in horizontalAnglesTrig.indices.dropLast() {
                let innerLeft = UInt32(2 * i)
                let outerLeft = UInt32(2 * i + 1)
                let innerRight = UInt32(2 * i + 2)
                let outerRight = UInt32(2 * i + 3)
                
                indices.append(contentsOf: [innerLeft, outerLeft, outerRight])
                indices.append(contentsOf: [innerLeft, outerRight, innerRight])
            }
            
            return .init(positions: positions, normals: normals, triangleIndices: indices)
        }()
        
        let bottomSurface: Submesh = {
            var positions = [simd_float3]()
            var normals = [simd_float3]()
            var indices = [UInt32]()
            
            let (sinVerticalAngle, cosVerticalAngle) = verticalAngleTrigLast
            ...
            for i in horizontalAnglesTrig.indices.dropLast() {
                ...
                indices.append(contentsOf: [outerLeft, innerLeft, innerRight])
                indices.append(contentsOf: [outerLeft, innerRight, outerRight])
            }
            
            return .init(positions: positions, normals: normals, triangleIndices: indices)
        }()
        
        let leftSurface: Submesh = {
            var positions = [simd_float3]()
            var normals = [simd_float3]()
            var indices = [UInt32]()
            
            let (sinHorizontalAngle, cosHorizontalAngle) = horizontalAngleTrigFirst
            
            for (sinVerticalAngle, cosVerticalAngle) in verticalAnglesTrig {
                ...
                let dx = sinVerticalAngle * -sinHorizontalAngle
                let dy = Float.zero
                let dz = cosVerticalAngle * -cosHorizontalAngle
            ...
            for i in verticalAnglesTrig.indices.dropLast() {
                let innerTop = UInt32(2 * i)
                let outerTop = UInt32(2 * i + 1)
                let innerBottom = UInt32(2 * i + 2)
                let outerBottom = UInt32(2 * i + 3)
                
                indices.append(contentsOf: [innerTop, innerBottom, outerBottom])
                indices.append(contentsOf: [innerTop, outerBottom, outerTop])
            }
            
            return .init(positions: positions, normals: normals, triangleIndices: indices)
        }()
        
        let rightSurface: Submesh = {
            var positions = [simd_float3]()
            var normals = [simd_float3]()
            var indices = [UInt32]()
            
            let (sinHorizontalAngle, cosHorizontalAngle) = horizontalAngleTrigLast
            ...
            for i in verticalAnglesTrig.indices.dropLast() {
                ...
                indices.append(contentsOf: [outerTop, outerBottom, innerBottom])
                indices.append(contentsOf: [outerTop, innerBottom, innerTop])
            }
            
            return .init(positions: positions, normals: normals, triangleIndices: indices)
        }()
        
        let outerSurface: Submesh = {
            var positions = [simd_float3]()
            var normals = [simd_float3]()
            var indices = [UInt32]()
            ...
            for (sinVerticalAngle, cosVerticalAngle) in verticalAnglesTrig {
                ...
                for (sinHorizontalAngle, cosHorizontalAngle) in horizontalAnglesTrig {
                    ...
                    let normal = simd_float3(x, y, z)
                    let position = outerRadius * normal
            ...
            let horizontalCount = horizontalAnglesTrig.count
            for v in verticalAnglesTrig.indices.dropLast() {
                for h in horizontalAnglesTrig.indices.dropLast() {
                    let upperLeft = UInt32(v * horizontalCount + h)
                    let lowerLeft = UInt32((v + 1) * horizontalCount + h)
                    let upperRight = UInt32(v * horizontalCount + h + 1)
                    let lowerRight = UInt32((v + 1) * horizontalCount + h + 1)
        ...
        let innerSurface: Submesh = {
            ...
            for (sinVerticalAngle, cosVerticalAngle) in verticalAnglesTrig {
                ...
                for (sinHorizontalAngle, cosHorizontalAngle) in horizontalAnglesTrig {
                    ...
                    let normal = -simd_float3(x, y, z)
                    let position = innerRadius * -normal
            ...
            for v in verticalAnglesTrig.indices.dropLast() {
                for h in horizontalAnglesTrig.indices.dropLast() {
                    ...
                    indices.append(contentsOf: [upperRight, lowerRight, lowerLeft])
                    indices.append(contentsOf: [upperRight, lowerLeft, upperLeft])
                }
            }
        ...
        return topSurface + bottomSurface + leftSurface + rightSurface + outerSurface + innerSurface
    }
    ...
    static func generateVolumetricSphericalPatch(
        ...
        return generateVolumetricSphericalPatch(radius: radius,
                                                horizontalAngle: horizontalAngle,
                                                verticalAngle: horizontalAngle / aspectRatio,
                                                unitAngle: unitAngle,
    ...
    static func generateSphericalPatches(
        radius: Float,
        horizontalAngle: Float,
        verticalAngle: Float,
        skipAngle: Float,
        ...
        let patch0 = generateSphericalPatch(radius: radius,
                                            horizontalAngle: horizontalAngle,
                                            verticalAngle: verticalAngle,
                                            unitAngle: unitAngle)
        let patches = (1..<count).map { index in
            return patch0.rotated(angle: skipAngle * Float(index), axis: .init(0, 1, 0))
        }
        return patches.reduce(patch0, +)
    }
    ...
    static func generateSphericalPatches(
        ...
        return generateSphericalPatches(radius: radius,
                                        horizontalAngle: horizontalAngle,
                                        verticalAngle: horizontalAngle / aspectRatio,
                                        skipAngle: skipAngle,
                                        count: count,
    ...
    static func generateVolumetricSphericalPatches(
        radius: Float,
        verticalAngle: Float,
        count: Int,
        unitAngle: Float,
        ...
        let horizontalAngle = min(max(twoPi / Float(count), 0), twoPi)
        let innerSurfacePatches = generateSphericalPatches(radius: radius - outlineMargin,
                                                           horizontalAngle: horizontalAngle,
                                                           verticalAngle: verticalAngle,
                                                           skipAngle: horizontalAngle,
                                                           count: count,
                                                           unitAngle: unitAngle).inverted
        let outerSurfacePatches = generateSphericalPatches(radius: radius + outlineMargin,
                                                           horizontalAngle: horizontalAngle,
                                                           verticalAngle: verticalAngle,
                                                           skipAngle: horizontalAngle,
                                                           count: count,
        ...
        let verticalAngleCount = Int((verticalAngle / unitAngle).rounded(.towardZero))
        let verticalHalfAngle = verticalAngle * 0.5 - residualVerticalAngle
        var verticalAngles = (0...verticalAngleCount).map { unitAngle * Float($0) - verticalHalfAngle + halfPi }
        if verticalAngleHasResidualAngle,
           let firstAngle = verticalAngles.first,
           let lastAngle = verticalAngles.last {
            verticalAngles = [firstAngle - residualVerticalAngle] +
        ...
        let topSurfaces = (1..<count).map { index in
            return topSurface.rotated(angle: horizontalAngle * Float(index), axis: .init(0, 1, 0))
        }.reduce(topSurface, +)
        ...
        let bottomSurfaces = (1..<count).map { index in
            return bottomSurface.rotated(angle: horizontalAngle * Float(index), axis: .init(0, 1, 0))
        }.reduce(bottomSurface, +)
        
        return innerSurfacePatches + outerSurfacePatches + topSurfaces + bottomSurfaces
    }
    ...
    static func generateVolumetricSphericalPatches(
        ...
        let horizontalLength = 2.0 * Float.pi * radius / Float(count)
        let verticalLength = horizontalLength / aspectRatio
        let verticalAngle = verticalLength / radius
        return generateVolumetricSphericalPatches(radius: radius,
                                                  verticalAngle: verticalAngle,
                                                  count: count,
                                                  unitAngle: unitAngle,
                                                  outlineMargin: outlineMargin)
...
extension Submesh {
    ...
    static func generateCylindricalPatch(
        radius: Float,
        horizontalAngle: Float,
        ...
        let unitLength = unitAngle * radius
        let residualVerticalLength = verticalLength.truncatingRemainder(dividingBy: unitLength) * 0.5
        let verticalLengthHasResidualLength = residualVerticalLength >= .pi / 180
        let verticalLengthCount = Int((verticalLength / unitLength).rounded(.towardZero))
        let verticalHalfLength = verticalLength * 0.5 - residualVerticalLength
        var verticalLengths = (0...verticalLengthCount).map { verticalHalfLength - unitLength * Float($0) }
        if verticalLengthHasResidualLength,
           let firstLength = verticalLengths.first,
           let lastLength = verticalLengths.last {
            verticalLengths = [firstLength - residualVerticalLength] +
            verticalLengths + [lastLength + residualVerticalLength]
        }
        ...
        let firstVerticalLength = verticalLengths.first!
        let lastVerticalLength = verticalLengths.last!
        let diffVerticalLength = lastVerticalLength - firstVerticalLength
        
        let firstHorizontalAngle = horizontalAngles.first!
        let lastHorizontalAngle = horizontalAngles.last!
        let diffHorizontalAngle = lastHorizontalAngle - firstHorizontalAngle
        
        for verticalLength in verticalLengths {
            
            let v = 1.0 - (verticalLength - firstVerticalLength) / diffVerticalLength
            
            for horizontalAngle in horizontalAngles {
                let sinHorizontalAngle = sin(horizontalAngle)
                let cosHorizontalAngle = cos(horizontalAngle)
                
                let x = cosHorizontalAngle
                let y = verticalLength
                let z = -sinHorizontalAngle
                
                let position = simd_float3(radius * x, y, radius * z)
                let normal = simd_float3(x, 0, z)
                ...
                let texcoord = rotate ? simd_float2(v, 1.0 - u) : simd_float2(u, v)
                texcoords.append(texcoord)
            }
        }
        ...
        for v in verticalLengths.indices.dropLast() {
            for h in horizontalAngles.indices.dropLast() {
                let upperLeft = UInt32(v * vertexCountPerRow + h)
                let lowerLeft = UInt32((v + 1) * vertexCountPerRow + h)
                let lowerRight = UInt32((v + 1) * vertexCountPerRow + (h + 1))
    ...
    static func generateCylindricalPatch(
        ...
        return generateCylindricalPatch(radius: radius,
                                        horizontalAngle: horizontalAngle,
                                        verticalLength: horizontalAngle * radius / aspectRatio,
                                        unitAngle: unitAngle,
                                        rotate: rotate)
    }
    
    static func generateVolumetricCylindricalPatch(
        radius: Float,
        horizontalAngle: Float,
        verticalLength: Float,
        unitAngle: Float,
        ...
        let unitLength = unitAngle * radius
//        let verticalLength = horizontalAngle * radius / aspectRatio
        let residualVerticalLength = verticalLength.truncatingRemainder(dividingBy: unitLength) * 0.5
        let verticalLengthHasResidualLength = residualVerticalLength >= .pi / 180
        let verticalLengthCount = Int((verticalLength / unitLength).rounded(.towardZero))
        let verticalHalfLength = verticalLength * 0.5 - residualVerticalLength
        ...
        if let firstLength = verticalLengths.first,
           let lastLength = verticalLengths.last {
            verticalLengths = [firstLength - outlineMargin] +
            verticalLengths + [lastLength + outlineMargin]
        }
        
        let horizontalAnglesTrig: [simd_float2] = horizontalAngles.map { simd_float2(sin($0), cos($0)) }
        
        let horizontalAngleTrigFirst = horizontalAnglesTrig.first!
        let horizontalAngleTrigLast = horizontalAnglesTrig.last!
        let verticalLengthFirst = verticalLengths.first!
        let verticalLengthLast = verticalLengths.last!
        ...
        let topSurface: Submesh = {
            ...
            let verticalLength = verticalLengthFirst
            
            for horizontalAngleTrig in horizontalAnglesTrig {
                let sinHorizontalAngle = horizontalAngleTrig.x
                let cosHorizontalAngle = horizontalAngleTrig.y
                
                let x = cosHorizontalAngle
                let y = verticalLength
                let z = -sinHorizontalAngle
                
                let innerPosition = simd_float3(innerRadius * x, y, innerRadius * z)
                let outerPosition = simd_float3(outerRadius * x, y, outerRadius * z)
        ...
        let bottomSurface: Submesh = {
            ...
            let verticalLength = verticalLengthLast
        ...
        let leftSurface: Submesh = {
            ...
            let sinHorizontalAngle = horizontalAngleTrigFirst.x
            let cosHorizontalAngle = horizontalAngleTrigFirst.y
            ...
            for verticalLength in verticalLengths {
                ...
                let dx = sinHorizontalAngle
                let dy = Float.zero
                let dz = cosHorizontalAngle
            ...
            for i in verticalLengths.indices.dropLast() {
                let innerTop = UInt32(2 * i)
                let outerTop = UInt32(2 * i + 1)
                let innerBottom = UInt32(2 * i + 2)
                let outerBottom = UInt32(2 * i + 3)
        ...
        let rightSurface: Submesh = {
            ...
            let sinHorizontalAngle = horizontalAngleTrigLast.x
            let cosHorizontalAngle = horizontalAngleTrigLast.y
        ...
        let outerSurface: Submesh = {
            ...
            for verticalLength in verticalLengths {
                ...
                for horizontalAngleTrig in horizontalAnglesTrig {
                    ...
                    let position = simd_float3(outerRadius * x, y, outerRadius * z)
                    let normal = simd_float3(x, 0, z)
        ...
        let innerSurface: Submesh = {
            ...
            for verticalLength in verticalLengths {
                ...
                for horizontalAngleTrig in horizontalAnglesTrig {
                    ...
                    let position = simd_float3(innerRadius * x, y, innerRadius * z)
                    let normal = -simd_float3(x, 0, z)
    ...
    static func generateVolumetricCylindricalPatch(
        ...
        return generateVolumetricCylindricalPatch(radius: radius,
                                                  horizontalAngle: horizontalAngle,
                                                  verticalLength: horizontalAngle * radius / aspectRatio,
                                                  unitAngle: unitAngle,
                                                  outlineMargin: outlineMargin)
    ...
    static func generateCylindricalPatches(
        radius: Float,
        horizontalAngle: Float,
        verticalLength: Float,
        skipAngle: Float,
        ...
        let patch0 = generateCylindricalPatch(radius: radius,
                                              horizontalAngle: horizontalAngle,
    ...
    static func generateCylindricalPatches(
        ...
        return generateCylindricalPatches(radius: radius,
                                          horizontalAngle: horizontalAngle,
                                          verticalLength: horizontalAngle * radius / aspectRatio,
                                          skipAngle: skipAngle,
                                          count: count,
    ...
    static func generateVolumetricCylindricalPatches(
        radius: Float,
        verticalLength: Float,
        count: Int,
        unitAngle: Float,
        ...
        let unitLength = unitAngle * radius
        let residualVerticalLength = verticalLength.truncatingRemainder(dividingBy: unitLength) * 0.5
        let verticalLengthHasResidualLength = residualVerticalLength >= 0.001
        let verticalLengthCount = Int((verticalLength / unitLength).rounded(.towardZero))
        let verticalHalfLength = verticalLength * 0.5 - residualVerticalLength
        var verticalLengths = (0...verticalLengthCount).map { verticalHalfLength - unitLength * Float($0) }
        if verticalLengthHasResidualLength,
        ...
        let innerSurface: Submesh = {
            ...
            let vertexCountPerRow = horizontalAnglesTrig.count
            ...
            return .init(positions: positions, normals: normals, triangleIndices: indices).inverted
        }()
        let innerSurfaces = (1..<count).map { index in
            return innerSurface.rotated(angle: horizontalAngle * Float(index), axis: .init(0, 1, 0))
        }.reduce(innerSurface, +)
        ...
        let outerSurfaces = (1..<count).map { index in
            return outerSurface.rotated(angle: horizontalAngle * Float(index), axis: .init(0, 1, 0))
        }.reduce(outerSurface, +)
        ...
        return innerSurfaces + outerSurfaces + topSurfaces + bottomSurfaces
    }
    ...
    static func generateVolumetricCylindricalPatches(
        ...
        return generateVolumetricCylindricalPatches(radius: radius,
                                                    verticalLength: 2.0 * Float.pi * radius / (Float(count) * aspectRatio),
                                                    count: count,
                                                    unitAngle: unitAngle,
                                                    outlineMargin: outlineMargin)
    }
...
enum TextureCoordinateRotation: Hashable, Codable {
    case none
    case clockwise90
    case upsideDown
...
extension Submesh {
    ...
    static func generateConicalPatch(
        topRadius: Float,
        bottomRadius: Float,
        horizontalAngle: Float,
        topHeight: Float,
        bottomHeight: Float,
        unitAngle: Float,
        rotate: TextureCoordinateRotation = .none
    ) -> Submesh {
        ...
        let unitLength = unitAngle * (topRadius + bottomRadius) * 0.5
        let verticalLength = topHeight - bottomHeight
        let residualVerticalLength = verticalLength.truncatingRemainder(dividingBy: unitLength) * 0.5
        let verticalLengthHasResidualLength = residualVerticalLength >= 0.001
        let verticalLengthCount = Int((verticalLength / unitLength).rounded(.towardZero))
        let verticalHalfLength = verticalLength * 0.5 - residualVerticalLength
        ...
        let diffRadius = topRadius - bottomRadius
        let diffHeight = topHeight - bottomHeight
        let oneOverSqrtDiffRH = 1 / sqrt(diffHeight * diffHeight + diffRadius * diffRadius)
        
        for verticalLength in verticalLengths {
            
            let v = 1.0 - (verticalLength - firstVerticalLength) / diffVerticalLength
            let radius = topRadius * v + bottomRadius * (1.0 - v)
            
            for horizontalAngle in horizontalAngles {
                let sinHorizontalAngle = sin(horizontalAngle)
                let cosHorizontalAngle = cos(horizontalAngle)
                
                let x = radius * cosHorizontalAngle
                let y = verticalLength
                let z = radius * -sinHorizontalAngle
                
                let position = simd_float3(x, y, z)
                let normal = simd_float3(cosHorizontalAngle * diffHeight,
                                         diffRadius,
                                         -sinHorizontalAngle * diffHeight) * oneOverSqrtDiffRH
                
                let u = (horizontalAngle - firstHorizontalAngle) / diffHorizontalAngle
                var texcoord = simd_float2(u, v)
                switch rotate {
                case .none: break
                case .clockwise90:
                    texcoord = simd_float2(1 - texcoord.y, texcoord.x)
                case .upsideDown:
                    texcoord = 1 - texcoord
                case .counterClockwise90:
                    texcoord = simd_float2(texcoord.y, 1 - texcoord.x)
                }
    ...
    static func generateConicalPatches(
        topRadius: Float,
        bottomRadius: Float,
        horizontalAngle: Float,
        topHeight: Float,
        ...
        let patch0 = generateConicalPatch(topRadius: topRadius,
...
extension Submesh {
    ...
    static func generateToricPatch(
        meanRadius: Float,
        tubeRadius: Float,
        verticalCenterAngle: Float,
        horizontalAngle: Float,
        verticalAngle: Float,
        unitAngle: Float,
        rotate: TextureCoordinateRotation = .none
...
//        let verticalAngle = Float.twoPi / 3
        ...
        let verticalAngleCount = Int((verticalAngle / unitAngle).rounded(.towardZero))
        let verticalHalfAngle = verticalAngle * 0.5 - residualVerticalAngle
        var verticalAngles = (0...verticalAngleCount).map { unitAngle * Float($0) - verticalHalfAngle + verticalCenterAngle }
        if verticalAngleHasResidualAngle,
           let firstAngle = verticalAngles.first,
           let lastAngle = verticalAngles.last {
            verticalAngles = [firstAngle - residualVerticalAngle] +
...
//        let screenAspectRatio = rotate == .counterClockwise90 || rotate == .clockwise90 ? 1 / aspectRatio : aspectRatio
//        let screenAspectRatio = aspectRatio
        
//        let horizontalAngle = verticalAngle * screenAspectRatio
        let residualHorizontalAngle = horizontalAngle.truncatingRemainder(dividingBy: unitAngle) * 0.5
        let horizontalAngleHasResidualAngle = residualHorizontalAngle >= .pi / 180
        let horizontalAngleCount = Int((horizontalAngle / unitAngle).rounded(.towardZero))
        let horizontalHalfAngle = horizontalAngle * 0.5 - residualHorizontalAngle
        ...
        for verticalAngle in verticalAngles {
            ...
            for horizontalAngle in horizontalAngles {
                ...
                let r = simd_float3(cosHorizontalAngle, 0, -sinHorizontalAngle)
                let tubeCenter = meanRadius * r
                let normal = r * cosVerticalAngle - .init(0, sinVerticalAngle, 0)
                let position = tubeCenter + tubeRadius * normal
                
                var texcoord = switch rotate {
                case .none:                 simd_float2(u, v)
                case .clockwise90:          simd_float2(1 - v, u)
                case .upsideDown:           simd_float2(1 - u, 1 - v)
                case .counterClockwise90:   simd_float2(v, 1 - u)
                }
    ...
    static func generateVolumetricToricPatch(
        meanRadius: Float,
        tubeRadius: Float,
        verticalCenterAngle: Float,
//        aspectRatio: Float,
...
//        rotate: TextureCoordinateRotation = .none
        ...
        let outlineMarginVerticalAngle = outlineMargin / tubeRadius
        let outlineMarginHorizontalAngle = outlineMargin / meanRadius
...
//        let verticalAngle = Float.twoPi / 3
        ...
        if let firstAngle = verticalAngles.first,
           let lastAngle = verticalAngles.last {
            verticalAngles = [firstAngle - outlineMarginVerticalAngle] +
            verticalAngles + [lastAngle + outlineMarginVerticalAngle]
        }
...
//        let horizontalAngle = verticalAngle * screenAspectRatio
        ...
        if let firstAngle = horizontalAngles.first,
           let lastAngle = horizontalAngles.last {
            horizontalAngles = [firstAngle - outlineMarginHorizontalAngle] +
            horizontalAngles + [lastAngle + outlineMarginHorizontalAngle]
        }
        ...
        let outerTubeRadius = tubeRadius + outlineMargin
        let innerTubeRadius = tubeRadius - outlineMargin
        ...
        let topSurface: Submesh = {
            ...
            for (sinHorizontalAngle, cosHorizontalAngle) in horizontalAnglesTrig {
                ...
                let r = simd_float3(cosHorizontalAngle, 0, -sinHorizontalAngle)
                let tubeCenter = meanRadius * r
                let n = r * cosVerticalAngle - .init(0, sinVerticalAngle, 0)
                let outerPosition = tubeCenter + outerTubeRadius * n
                let innerPosition = tubeCenter + innerTubeRadius * n
                
                let normal = normalize(r * sinVerticalAngle + .init(0, cosVerticalAngle, 0))
        ...
        let leftSurface: Submesh = {
            ...
            for (sinVerticalAngle, cosVerticalAngle) in verticalAnglesTrig {
                ...
                let normal = simd_float3(sinHorizontalAngle, 0, cosHorizontalAngle)
