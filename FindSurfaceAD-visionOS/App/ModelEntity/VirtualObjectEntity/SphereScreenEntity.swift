...
fileprivate let panelVerticalAngle: Float = panelHorizontalAngle * 9.0 / 16.0
...
final class SphereScreenEntity: VirtualObjectEntity, HasPlayableComponent, HasCustomHighlightComponent {
    ...
    required init() {
        ...
        let occlusion: ModelEntity = {
            let mesh = MeshResource.generateSphere(name: "sphere", radius: 1.0)
    ...
    func update(block: (inout Intrinsics) -> Void) {
        ...
        let aspectRatio = Float(intrinsics.resource?.thumbnail.size.aspectRatio ?? 1)
        
        let panelMesh: MeshResource = {
            let submesh = Submesh.generateSphericalPatches(radius: radius,
        ...
        let panelOutlineMesh: MeshResource = {
            ...
            let submesh = Submesh.generateVolumetricSphericalPatches(radius: radius,
                                                                     aspectRatio: aspectRatio,
                                                                     count: 3,
                                                                     unitAngle: .pi / 36,
        ...
        let screenMesh: MeshResource = {
            ...
            let verticalLength = (panelHorizontalAngle / aspectRatio) * (radius + 0.001) - screenMargin * 2
            let verticalAngle = verticalLength / (radius + 0.001)
            let submesh = Submesh.generateSphericalPatches(radius: radius + 0.001,
                                                           horizontalAngle: horizontalAngle,
                                                           aspectRatio: horizontalAngle / verticalAngle,
                                                           skipAngle: panelHorizontalAngle,
                                                           count: 3,
                                                           unitAngle: .pi / 36)
            return try! MeshResource.generate(from: submesh)
        ...
        occlusion.scale = .init(repeating: _radius)
